...
class Workflow():
    def __init__(self):
        ...
        workflow.add_node("email_writer", nodes.write_draft_email)
        workflow.add_node("email_proofreader", nodes.verify_generated_email)
        workflow.add_node("send_email", nodes.send_email_response)  # Updated to use send_email_response
        ...
        workflow.add_conditional_edges(
            ...
            nodes.must_rewrite,
            {
                "send": "send_email",  # Change here to go directly to send_email
                "rewrite": "email_writer",
                "stop": "categorize_email"
            }
        )
        ...
        # check if there are still emails to be processed
        workflow.add_edge("send_email", "is_email_inbox_empty")  # Check for more emails after sending
        workflow.add_edge("skip_unrelated_email", "is_email_inbox_empty" )
